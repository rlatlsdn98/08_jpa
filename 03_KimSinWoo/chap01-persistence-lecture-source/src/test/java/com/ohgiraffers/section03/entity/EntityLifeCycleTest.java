...
class EntityLifeCycleTest {
    ...
    void testTransient(int menuCode) {
        ...
        // foundMenu : Entity Manager를 통해 DB에서 조회한 객체 -> 영속 상태
        Menu foundMenu = lifeCycle.findMenuByMenuCode(menuCode);

        // newMenu : 동일한 값을 가지지만 직접 생성한 새 객체 -> 비영속 상태
    ...
    void testManagedOtherEntityManager(int menuCode) {
        ...
        // 새로운 Entity가 생성됨 -> 서로 독립된 영속성 컨텍스트를 관리
    ...
    void testDetachEntity(int menuCode, int menuPrice) {
        ...
        // If 영속 상태 -> tbl_menu에서 menu_code가 일치하는 행의 price가 수정됨
        //  (더티 체킹: @Entity 객체의 변화(DB와 스탭샷 차이)를 감지)

        // (현재)준영속 상태 -> DB table과 매핑 X, 그냥 자바 객체의 값만 변경
        // (더티 체킹X)
    ...
    void testDetachAndMerge(int menuCode, int menuPrice) {
        ...
        // 준영속 상태에서 foundMenu.MenuPrice 변경
        // -> 더티 체킹의 대상이 아니기 때문에 DB값 수정이 되지 않는다
        foundMenu.setMenuPrice(menuPrice);
         ...
         */
        entityManager.merge(foundMenu); // 준영속 -> 영속 상태 변경
    ...
    void testMergeUpdate(int menuCode, String menuName) {
        ...
        Menu foundMenu = entityManager.find(Menu.class, menuCode);
        entityManager.detach(foundMenu); // foundMenu 영속 -> 준영속 상태로 변경
        ...
        // then
        assertEquals(refoundMenu.getMenuName(), mergeMenu.getMenuName());
    ...
    void testMergeSave() {
        ...
        // -> 더티 체킹 -> menu_code(PK)가 999인 행이 없음을 확인함
        // INSERT 구문 생성.
